...
const SOLANA_CLUSTER_ENDPOINT = 'https://fragrant-billowing-haze.solana-mainnet.quiknode.pro/'
...
export class SolanaClient {
  ...
  async (wallets: string[], author: string = '', tokenName: string = ''): Promise<any> => {
    try {  
      ...
      const nfts = await Promise.all(
        potentialNFTsByOwnerAddress.map(async ({ mintAddresses }) => {
          ...
          for (let i = 0; i < nonNullInfos.length; i ++) {
            ...
            if ((metadata.updateAuthority == author) && metadata?.data?.name.indexOf(tokenName) >= 0) {
                metadataList.push({
                  ...metadata
                });
